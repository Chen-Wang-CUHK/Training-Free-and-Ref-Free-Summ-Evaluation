...
def get_weights(sent_info_dic, sent_vecs, metric, pacsum_beta=0.0, pacsum_lambda1=2.0, pacsum_lambda2=1.0):
    if metric == 'full_doc':
        weights = [1.]*len(sent_info_dic)
    elif metric.startswith('indep_pacsum'):
        num = 0
        if 'top' in metric.lower():
            eles = metric.split('_')
            # 'indep_pacsum_topN'
            num = int(eles[2][3:])
        weights = get_indep_pacsum_weights(sent_info_dic, sent_vecs, num,
...
def parse_documents(docs, bert_model, ref_metric, sent_represnt_type='mean_all', pacsum_beta=0.0, pacsum_lambda1=2.0, pacsum_lambda2=1.0):
    if ref_metric == 'true_ref':
        sent_info_dic, sent_vecs, token_vecs, all_tokens = parse_refs(docs,bert_model,sent_represnt_type)
        sents_weights = [1.]*len(sent_info_dic)
    else:
        sent_info_dic, sent_vecs, token_vecs, all_tokens = parse_docs(docs,bert_model,sent_represnt_type)
    ...
    # print('=====pseudo ref=====')
    # print('\n'.join(pseudo_ref))
    return sent_info_dic, sent_vecs, sents_weights, token_vecs, all_tokens
