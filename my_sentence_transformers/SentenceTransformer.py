...
from utils import get_sent_vecs
...
class SentenceTransformer(nn.Sequential):
    ...
    def encode(self, sentences: List[str], sent_represnt_type: str = 'mean_all', batch_size: int = 8, show_progress_bar: bool = None, token_vecs: bool = False) -> Tuple[list, list, list]:
        """
        Computes sentence embeddings
        :param sentences:
           the sentences to embed
        :param sent_represnt_type:
           the method to get the sentence level representations
        :param batch_size:
        ...
        :return:
           a tuple with the embeddings for each sentence, the embeddings for each token, the tokens
        """
        # if torch.cuda.is_available():
        #     batch_size = 128 # use a large batch size when GPU is available, add by wchen
        ...
        all_tokens = []
        if len(sentences) == 0:
            # all_embeddings, all_token_embeddings, all_tokens
        ...
        for batch_idx in iterator:
            ...
            with torch.no_grad():
                ...
                for i,l in enumerate(embeddings['sentence_lengths'].to('cpu').numpy()):
                    ...
                    batch_token_embeds.append(raw_token_vecs[i][:l])
                    stokens = self._first_module().ids_to_tokens(token_ids[i][:l])
                    all_tokens.append(stokens)
                    batch_tokens.append(stokens)
                # add by wchen to add the selection of the sent_represnt_type
                sent_embeddings = get_sent_vecs([batch_token_embeds], [batch_tokens], sent_represnt_type, no_stack=True)
                all_embeddings.extend(sent_embeddings[0])
        ...
        return all_embeddings, all_token_embeddings, all_tokens
